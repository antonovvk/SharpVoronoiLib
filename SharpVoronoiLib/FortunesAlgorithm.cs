...
namespace SharpVoronoiLib
{
    internal class FortunesAlgorithm
    {
        ...
        public FortunesAlgorithm(VoronoiPlane plane)
        {
        ...
        internal LinkedList<VoronoiEdge> Run(BorderEdgeGeneration borderGeneration)
        {
            MinHeap<FortuneEvent> eventQueue = new MinHeap<FortuneEvent>(5 * _plane.Sites.Count());

            foreach (VoronoiSite site in _plane.Sites)
            {
                if (site == null) throw new ArgumentNullException(nameof(_plane.Sites));
            ...
            while (edgeNode != null)
            {
                ...
                bool valid = ClipEdge(edge, _plane.MinX, _plane.MinY, _plane.MaxX, _plane.MaxY);
                if (valid)
                {
            ...
            if (borderGeneration == BorderEdgeGeneration.MakeBorderEdges)
                CloseBorders(edges, _plane.MinX, _plane.MinY, _plane.MaxX, _plane.MaxY, _plane.Sites);
